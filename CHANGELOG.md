...
- **修复应用内授权窗口秒退（ERR_FAILED）**：OpenAI 的授权 URL 会立刻 302 跳到登录页，导致 Electron 的 `loadURL` 以 `ERR_FAILED` reject——之前把这个正常的跳转当成致命错误、直接关窗。现在忽略 `loadURL` 的 reject，授权靠导航拦截驱动，窗口正常显示登录页。
- **按端口是否可用自动选授权方式**：本机回调端口（OpenAI 固定 1455）能绑上（没被 Docker/WSL 占）就走「系统浏览器 + 自动回调」——登录完自动同步、无需任何手动操作；绑不上才用「应用内窗口拦截回调」。两条路都不用再手填回调地址。

## 0.4.4

- **OAuth 应用内一键授权**（学 Cockpit 做法）：OpenAI / Kiro / Windsurf / Antigravity 授权改为在应用内弹出的登录窗口里完成，直接拦截回调 URL 拿到 code，自动同步账号——不再依赖本机 1455 端口、不再需要手动粘贴回调地址。端口被系统占用（Docker/WSL）也照样能授权。Google/GitHub 登录被内嵌窗口拦截时，可折叠区仍提供「浏览器 + 手动回调」兜底。
- **Kiro 卡片信息更全**：额度条下显示 `已用 / 上限 · 剩余` 的具体积分数（对齐 Cockpit 的 `11,038 / 1,000 · 0 left` 密度），试用/奖励额度同样带数字。
- 新授权的 Kiro 账号会带出邮箱（从 id_token 解析）；旧账号重新授权一次即可显示。
